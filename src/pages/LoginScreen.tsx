...
const LoginScreen: React.FC = () => {
    ...
    /**
     * Handles the process of generating an OTP (One-Time Password) by invoking a command and updating the UI state.
     *
     * This function performs the following steps:
     * 1. Initiates the loading state to indicate the OTP request process is in progress.
     * 2. Validates the mobile number. If invalid, the function terminates the process and resets the loading state.
     * 3. Sends a request to generate an OTP by invoking the `request_otp_command` with the provided mobile number.
     * 4. On a successful response, it reveals the OTP input field and updates the request status.
     * 5. Manages any failures by updating the state to reflect the unsuccessful OTP request.
     * 6. Captures and logs any exceptions that occur during the OTP request process.
     * 7. Upon completion of the OTP request, resets the loading state and triggers a snackbar notification with the response message.
     *
     * @async
     * @function handleGenerateOTP
     * @returns {Promise<void>} - A promise that resolves when the OTP request and UI updates are complete.
     */
            ...
            <div className="flex flex-col h-screen items-center justify-center">
                <div
                    ...
                    <div
                        ...
                        {showOtp &&
                            <p
                                className="text-primary-blue text-[1.5vw] lg:text-[1vw] font-bold !select-none cursor-pointer">Resend
                                OTP</p>
                        ...
                        <div className="flex flex-col items-center justify-center select-none">
                            ...
                            <div className={`${isRequestOtpFails ? 'block' : 'hidden'}`}>
                                <p className={`text-secondary-green text-xs pt-6 mx-3 font-bold text-center`}>Sorry, But you
                                    are
                                    not
                                    connected with us
                                    <span className={`text-primary-blue cursor-pointer`}> Become our partner</span> or <span
                                        className={`text-primary-blue cursor-pointer`}>Download our app</span>
                                </p>
